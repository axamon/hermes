...
# $myAVSFile = $myAVSDataDir . 'Fruizioni_AVS_30112018.zip';                                                    # File contenente i dati di AVS (Front End di Tim Vision)
 $myAVSFile = $myAVSDataDir . 'Fruizioni_30112018.csv';
...
# LOADLOOP: while ($line = <$fh>) {
    ...
    #    chomp $line;
                ...
                if ($CLI =~ /^[0-9]+/){                                                                                                 # Verifica la presenza dell'indirizzo IP
                        ...
                        $Outputline = "$fields[0]|$fields[1]|$fields[2]|$fields[3]|$fields[5]|$fields[12]|$fields[13]|$fields[16]|$fields[17]\n";        # Linea criptata, esclusi i campi non di interesse                                                         # Costruisce la riga per la scrittura nel file risultato
                        # $PrintedChars += $gz_out->gzwrite($Outputline)
                        ...
                        if ($ChunkCount > 100) {die 'Test eseguito con successo';}
                }
